...
/* https://leetcode.com/problems/remove-duplicates-from-sorted-list/
  Given a linked list, remove all duplicate values.
...
MyLinkedList.prototype.deleteDuplicates = function() {
    ...
    while(current !== null){ // while the current head value isn't null, iterate.

        if(current.next !== null && current.next.val === current.val){ // if the next value is equal to the current value (and isn't null)
            ahead = current; // initialise ahead to current.

            while(ahead !== null && current.val === ahead.val){ // Set ahead to the next value, until the value of ahead isn't equal to current.
                ahead = ahead.next // by the end of this while loop, ahead will be equal to the node after the duplicate nodes.
            }

            current.next = ahead // setting next value of current to equal ahead (the value after all the duplicates)
        }
        current = current.next // moving to the next value to check
    }
